...
async function init(){
 try{
  ...
  try{await db.query("ALTER TABLE deposits MODIFY screenshot LONGTEXT")}catch(e){try{await db.query("ALTER TABLE deposits ADD COLUMN screenshot LONGTEXT")}catch(e2){}}}
...
app.post('/api/deposit',async(req,res)=>{
 try{
  ...
  console.log("DEPOSIT REQ",userId,amount);
  ...
  res.json({ok:1});
 }catch(e){console.log("DEPOSIT ERR",e.message); res.status(500).json({error:e.message})}
});
...
app.get('/api/admin/deposits',async(req,res)=>{
 ...
 try{
  ...
  console.log("ADMIN deposits total",r.length,"pending",pending.length);
  res.json(pending.length?pending:r);
 }catch(e){res.json([])}
});
...
app.get('/dashboard',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:15px}button{width:100%;padding:14px;margin:6px 0;border-radius:10px;border:none;font-weight:bold}.gold{background:gold}.dark{background:#222;color:#fff}.bal{background:#111;padding:15px;border-radius:15px;text-align:center;margin-bottom:10px}.green{color:#0f0}</style></head><body><div class="bal"><h3>Balance</h3><h1 id="b">0 UGX</h1><p id="ph"></p><p>Interest Available: <span id="int" class="green">0 UGX</span></p><p id="code" style="font-size:12px;color:gold"></p><p style="font-size:11px;color:#888">Min withdraw 5k - Anytime</p></div><button class="gold" onclick="location.href='/deposit'">💵 Deposit</button><button class="gold" onclick="location.href='/invest'">📈 Invest in Clubs</button><button class="dark" onclick="withdraw()">💸 Withdraw Interest</button><button class="dark" onclick="location.href='/myinvest'">📊 My Investments</button><button class="dark" onclick="location.href='/withdraws'">📜 Withdraw History</button><button class="dark" onclick="localStorage.removeItem('uid');location.href='/'">Logout</button><script>let uid=localStorage.getItem('uid');if(!uid)location.href='/';async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();document.getElementById('b').textContent=(u.balance||0)+' UGX';document.getElementById('ph').textContent=u.phone||'';document.getElementById('int').textContent=(u.totalInterest||0)+' UGX';document.getElementById('code').textContent='Ref Code: '+(u.myReferralCode||'')+' | Earn 10%';}async function withdraw(){let a=prompt('Enter amount to withdraw (min 5000)');if(!a)return;let r=await fetch('/api/withdraw',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:uid,amount:parseInt(a)})});let j=await r.json();alert(j.error||'Withdraw request sent! Wait approval');}load()</script></body></html>`);});
app.get('/deposit',(req,res)=>{res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:20px}input,button{width:100%;padding:14px;margin:8px 0;border-radius:10px;border:none}button{background:gold;font-weight:bold}</style></head><body><a href="/dashboard" style="color:gold">← Back</a><h2>💵 Deposit</h2><input id="am" placeholder="Amount"><input id="air" placeholder="Your Airtel No"><input type="file" id="file" accept="image/*"><img id="prev" style="display:none;width:100%;margin:10px 0"><button onclick="dep()">Submit</button><script>let uid=localStorage.getItem("uid");let b64="";document.getElementById("file").addEventListener("change",e=>{let r=new FileReader();r.onload=()=>{b64=r.result;let im=document.getElementById("prev");im.src=b64;im.style.display="block"};r.readAsDataURL(e.target.files[0])});async function dep(){if(!b64){alert("Upload screenshot");return}let r=await fetch("/api/deposit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:uid,amount:am.value,airtelNo:air.value,screenshot:b64})});let j=await r.json();if(j.ok){alert("Sent! Now check Admin");location.href="/dashboard"}else alert("Error: "+j.error)}</script></body></html>');});
app.get('/invest',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:15px}.club{background:#111;padding:15px;border-radius:12px;margin:10px 0;border-left:4px solid gold} button{width:100%;padding:12px;border-radius:8px;border:none;background:gold;font-weight:bold;margin-top:8px} input{width:100%;padding:12px;border-radius:8px;border:none;margin:5px 0} a{color:gold}</style></head><body><a href="/dashboard">← Back</a><h2>📈 Invest in Football Clubs</h2><p>Balance: <span id="bal">0</span> UGX</p><div class="club" style="border-color:red"><h3>🔴 Arsenal F/C</h3><p>10% daily - Locked 10 days</p><input id="a-arsenal" placeholder="Amount"><button onclick="inv('arsenal')">Invest Arsenal</button></div><div class="club" style="border-color:#ff0000"><h3>🔴 Man United F/C</h3><p>10% daily - Locked 10 days</p><input id="a-manutd" placeholder="Amount"><button onclick="inv('manutd')">Invest Man Utd</button></div><div class="club" style="border-color:#6ec1e4"><h3>🔵 Man City F/C</h3><p>10% daily - Locked 10 days</p><input id="a-mancity" placeholder="Amount"><button onclick="inv('mancity')">Invest Man City</button></div><div class="club" style="border-color:#034694"><h3>🔵 Chelsea F/C</h3><p>8% daily - Locked 8 days</p><input id="a-chelsea" placeholder="Amount"><button onclick="inv('chelsea')">Invest Chelsea</button></div><div class="club" style="border-color:#c8102e"><h3>🔴 Liverpool F/C</h3><p>8% daily - Locked 8 days</p><input id="a-liverpool" placeholder="Amount"><button onclick="inv('liverpool')">Invest Liverpool</button></div><script>let uid=localStorage.getItem('uid');async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();document.getElementById('bal').textContent=u.balance}async function inv(club){let amt=document.getElementById('a-'+club).value;if(!amt)return alert('Enter amount');let r=await fetch('/api/invest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:uid,club,amount:parseInt(amt)})});let j=await r.json();if(j.ok){alert('Invested!');location.href='/myinvest'}else alert(j.error)}load()</script></body></html>`);});
app.get('/myinvest',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:15px}.card{background:#111;padding:12px;margin:10px 0;border-radius:10px}a{color:gold}</style></head><body><a href="/dashboard">← Back</a><h2>My Investments</h2><div id="l">Loading...</div><script>let uid=localStorage.getItem('uid');async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();let list=u.investments||[];let now=new Date();let html='';for(let i of list){let days=Math.floor((now-new Date(i.startDate))/(1000*60*60*24));let profit=Math.floor(i.amount*i.rate/100*days);let left=i.lockDays-days;html+='<div class="card"><b>'+i.club.toUpperCase()+'</b> - '+i.amount+' UGX<br>Rate: '+i.rate+'% daily | Lock: '+i.lockDays+' days<br>Running: '+days+' days<br>Profit: '+profit+' UGX<br>'+(left>0?'Locked - '+left+' days left':'✅ UNLOCKED')+'</div>'}document.getElementById('l').innerHTML=html||'No investments';}load()</script></body></html>`);});
app.get('/withdraws',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:15px}a{color:gold}.card{background:#111;padding:10px;margin:8px 0;border-radius:8px}</style></head><body><a href="/dashboard">← Back</a><h2>Withdraw History</h2><div id="l">Loading...</div><script>let uid=localStorage.getItem('uid');async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();document.getElementById('l').innerHTML='Interest Available: '+u.totalInterest+' UGX';}load()</script></body></html>`);});
app.get('/admin',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;padding:15px;font-family:Arial}input,button{width:100%;padding:10px;margin:5px 0;border-radius:8px;border:none}button{background:gold;font-weight:bold}.tab{padding:10px;background:#222;display:inline-block;margin:5px;border-radius:5px;cursor:pointer}.active{background:gold;color:#000}</style></head><body><div id="loginBox"><h2>Admin Login</h2><input id="pass" type="password" placeholder="Password"><button onclick="check()">Unlock</button><p id="err"></p></div><div id="adminBox" style="display:none"><h2>ADMIN PANEL</h2><div><span class="tab active" id="t1" onclick="showTab('dep')">Deposits</span><span class="tab" id="t2" onclick="showTab('with')">Withdraws</span></div><div id="depBox"><h3>Pending Deposits</h3><div id="l">Loading...</div></div><div id="withBox" style="display:none"><h3>Pending Withdraws</h3><div id="lw">Loading...</div></div></div><script>const ADMIN_PASS="LIFELINE123";let entered="";function check(){let v=document.getElementById("pass").value;if(v===ADMIN_PASS){entered=v;document.getElementById("loginBox").style.display="none";document.getElementById("adminBox").style.display="block";ld();lw()}else document.getElementById("err").textContent="Wrong"}function showTab(t){if(t==='dep'){document.getElementById('depBox').style.display='block';document.getElementById('withBox').style.display='none';document.getElementById('t1').classList.add('active');document.getElementById('t2').classList.remove('active')}else{document.getElementById('depBox').style.display='none';document.getElementById('withBox').style.display='block';document.getElementById('t2').classList.add('active');document.getElementById('t1').classList.remove('active');lw()}}async function ld(){let r=await fetch("/api/admin/deposits?key="+entered);let d=await r.json();console.log(d);let e=document.getElementById("l");if(!d.length){e.innerHTML="No pending - Total in DB: 0"}else{let h="Total: "+d.length+"<br>";for(let x of d){h+="<div id=\\"r-"+x.id+"\\" style=\\"background:#222;padding:12px;margin:10px 0;border-radius:10px\\"><b>"+x.phone+"</b> - "+x.amount+" UGX<br>From: "+x.airtelNo+" Status: "+x.status+"<br>"+(x.screenshot?"<img src=\\""+x.screenshot+"\\" style=\\"width:100%;max-width:300px;margin:10px 0\\">":"No img")+"<br><button onclick=\\"ap("+x.id+")\\" style=\\"background:lime;padding:8px\\">Approve ✅</button></div>"}e.innerHTML=h}}async function ap(id){await fetch("/api/admin/approve/"+id+"?key="+entered,{method:"POST"});let el=document.getElementById("r-"+id);el.style.opacity=0;setTimeout(()=>el.remove(),500)}async function lw(){let r=await fetch("/api/admin/withdraws?key="+entered);let d=await r.json();let e=document.getElementById("lw");if(!d.length){e.innerHTML="No pending withdraws"}else{let h="";for(let x of d){h+="<div id=\\"w-"+x.id+"\\" style=\\"background:#222;padding:12px;margin:10px 0;border-radius:10px\\"><b>"+x.phone+"</b> wants "+x.amount+" UGX<br><button onclick=\\"aw("+x.id+",'approve')\\" style=\\"background:lime;padding:8px;margin-right:5px\\">Approve ✅</button><button onclick=\\"aw("+x.id+",'reject')\\" style=\\"background:red;color:#fff;padding:8px\\">Reject</button></div>"}e.innerHTML=h}}async function aw(id,act){await fetch("/api/admin/withdraw/"+id+"?key="+entered,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:act})});let el=document.getElementById("w-"+id);el.style.opacity=0;setTimeout(()=>el.remove(),500)}</script></body></html>`);});
