...
// Create tables + add missing columns if old table exists
...
pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS referralCode VARCHAR(20) UNIQUE`);
pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS referredBy INT DEFAULT NULL`);
pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS totalReferralBonus DECIMAL(10,2) DEFAULT 0`);
pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS balance DECIMAL(10,2) DEFAULT 0`);
...
app.get('/', (req,res)=>{
  res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#0a0a0a;color:#fff;font-family:sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0}.box{background:#1a1a1a;padding:25px;border-radius:15px;width:90%;max-width:360px}h1{color:#00ff88;text-align:center}input{width:100%;padding:12px;margin:8px 0;border-radius:8px;border:none}button{width:100%;padding:12px;background:#00ff88;border:none;border-radius:8px;font-weight:bold;margin-top:10px}.refBox{background:#222;padding:10px;border-radius:8px;border:1px dashed #00ff88;margin:8px 0}</style></head><body><div class="box"><h1>LIFELINE INVESTMENTS</h1><p style="text-align:center;color:#ffaa00">Earn 5% Daily + 10% Referral Bonus!</p><h3>Register</h3><input id="name" placeholder="Full Name"><input id="phone" placeholder="Phone"><input id="pass" type="password" placeholder="Password"><div class="refBox"><input id="refCode" placeholder="Referral Code (Optional)"><small id="refInfo" style="color:#00ff88;display:none"></small><br><small style="color:#888">If referred, enter code</small></div><button onclick="reg()">Register</button><p id="msg"></p><hr><h3>Login</h3><input id="lphone" placeholder="Phone"><input id="lpass" type="password" placeholder="Password"><button onclick="log()">Login</button><p id="msg2"></p></div><script>
const urlParams = new URLSearchParams(window.location.search);
const refFromUrl = urlParams.get('ref');
if(refFromUrl){ document.getElementById('refCode').value = refFromUrl; document.getElementById('refInfo').style.display='block'; document.getElementById('refInfo').innerText='Referred by '+refFromUrl; }
...
app.post('/api/register', async (req,res)=>{
  ...
  if(referralCode && referralCode.trim()!==''){
    try{
      const [refUser] = await pool.promise().query('SELECT id FROM users WHERE referralCode=? OR phone=?',[referralCode.trim(), referralCode.trim()]);
      if(refUser.length>0) referredById = refUser[0].id;
    }catch(e){ console.log('ref lookup error', e.message); }
  }
  pool.query('INSERT INTO users (fullName,phone,password,referralCode,referredBy) VALUES (?,?,?,?,?)',[fullName,phone,hashed,myCode,referredById],(err)=>{
    if(err){
      console.log('register err', err.sqlMessage);
      if(err.code==='ER_DUP_ENTRY')return res.status(400).json({message:'Phone already registered or code exists, try again'});
  ...
  pool.query('SELECT balance FROM users WHERE id=?',[userId], (err, rows)=>{
    ...
    pool.query('UPDATE users SET balance=balance-? WHERE id=?',[amount, userId], ()=>{
      pool.query('INSERT INTO investments (userId,plan,amount,claimed,status) VALUES (?,?,?,?,?)',[userId, plan, amount, 0, 'active'], (err2)=>{
        ...
        res.json({message:'Invested '+amount+'! Earn 5% daily = '+(amount*0.05)+'/day'});
      });
    });
  });
});
app.post('/api/withdraw', (req,res)=>{
  ...
  pool.query('SELECT balance FROM users WHERE id=?',[userId], (err, rows)=>{
    if(err||!rows.length) return res.status(400).json({message:'User not found'});
    if(parseFloat(rows[0].balance) < parseFloat(amount)) return res.status(400).json({message:'Insufficient balance! Have '+rows[0].balance});
    pool.query('UPDATE users SET balance=balance-? WHERE id=?',[amount, userId], ()=>{
      pool.query('INSERT INTO withdraws (userId, amount, phone, status) VALUES (?,?,?,?)',[userId, amount, phone, 'pending'], (err2)=>{
        if(err2) return res.status(500).json({message:err2.sqlMessage});
        res.json({message:'Withdraw '+amount+' requested to '+phone});
      });
    });
  });
});
      ...
      pool.query('UPDATE deposits SET status=? WHERE id=?',['approved',req.params.id],()=>{
        if(dep.referredBy){
          ...
          pool.query('UPDATE users SET balance=balance+?, totalReferralBonus=totalReferralBonus+? WHERE id=?',[bonus, bonus, dep.referredBy], ()=>{
            res.json({message:'Approved! User +'+dep.amount+' | Referrer +10% = '+bonus});
          });
        } else {
          res.json({message:'Approved! No referrer.'});
        }
      });
    });
  });
