...
<div class="tableWrap"><h3>🏆 League Table - S<span id="tblSeason">1</span> MD <span id="tblMD">0</span>/15</h3><div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding-left:8px">Team</th><th>MP</th><th>W</th><th>D</th><th>L</th><th>GF</th><th>GA</th><th>Pts</th></tr></thead><tbody id="tableBody"><tr><td colspan="8" style="color:#666">Loading...</td></tr></tbody></table></div><div style="font-size:11px;color:#666;text-align:center;margin-top:8px">15 MDs per season • Every team plays each other once</div></div>
</div>
<div id="nextSec" style="display:none"><div style="padding:12px;color:#888;text-align:center;background:#1a1a1a;margin:0 12px;border-radius:10px">Next MD <span id="nextMD">2</span> in <span id="nextCountdown" style="color:#ff6b00">--:--</span></div><div id="nextList"></div></div>
...
let html='';sorted.forEach((t,i)=>{let c=i<4?'color:#0f0':i>12?'color:#f55':'';html+='<tr style="'+c+'"><td><span style="color:#666;margin-right:4px">'+(i+1)+'</span>'+t.short+'</td><td>'+t.pld+'</td><td>'+t.w+'</td><td>'+t.d+'</td><td>'+t.l+'</td><td>'+t.gf+'</td><td>'+t.ga+'</td><td style="font-weight:900;color:gold">'+t.pts+'</td></tr>';});
...
else{tag='<span style="color:#555;font-size:11px">BETTING</span>';}
...
async function loadResults(){try{let r=await fetch("/api/virtual/results");let d=await r.json();if(!d.length){resList.innerHTML='<div style="padding:30px;text-align:center;color:#666">No results yet</div>';return;}let h='';d.forEach(md=>{h+='<div class=resCard><div style="display:flex;justify-content:space-between;font-weight:900;color:gold;margin-bottom:8px"><span>S'+md.season+' MD '+String(md.matchday).padStart(2,'0')+'</span><span style="color:#888;font-size:12px">'+md.time+'</span></div>';md.fixtures.forEach(f=>{h+='<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #2a2a2a"><span>'+f.home.short+' vs '+f.away.short+'</span><b>'+f.score+'</b><span style="color:'+(f.result==='home'?'#0f0':f.result==='away'?'#f00':'#ff0')+'">'+f.result.toUpperCase()+'</span></div>';});h+='</div>';});resList.innerHTML=h;}catch(e){}}
async function sync(){try{let r=await fetch("/api/virtual/status");let d=await r.json();topMD.textContent=String(d.matchday).padStart(2,'0');topSeason.textContent=d.season;tblMD.textContent=d.matchday;tblSeason.textContent=d.season;curPhase=d.phase;let tl=d.timeLeft;let disp="";let label="";if(d.phase==='betting'){disp=fmtDown(tl);label='BETTING - '+disp+' ↓';}else if(d.phase==='first_half'){disp=fmtUp(45-tl);label='🔴 1ST HALF '+disp+' ↑';}else if(d.phase==='halftime'){disp="00:0"+tl;label='⏸️ HALFTIME '+disp+' ↓';}else{let total=45+(45-tl);disp=fmtUp(total);label='🔴 2ND HALF '+disp+' ↑';}
phaseInfo.textContent=label;timerTop.textContent=disp;let nextIn=d.phase==='betting'?d.timeLeft+95:d.phase==='first_half'?50+d.timeLeft:d.phase==='halftime'?45+d.timeLeft:d.timeLeft;nextCountdown.textContent=fmtDown(nextIn);nextMD.textContent=(d.matchday%15)+1;
...
app.listen(process.env.PORT||3000,()=>console.log("FINAL FINAL: Invest 10D 10% (3 teams) + 8D 8% (3 teams) + League table + Round-robin 15 MDs + max5 goals"));
