...
// HOME PAGE - registration page directly from backend
app.get('/', (req,res) => {
  res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#0a0a0a;color:white;font-family:sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0}.box{background:#1a1a1a;padding:25px;border-radius:15px;width:90%;max-width:350px} h1{color:#00ff88;text-align:center} input{width:100%;padding:12px;margin:8px 0;border-radius:8px;border:none;box-sizing:border-box} button{width:100%;padding:12px;background:#00ff88;border:none;border-radius:8px;font-weight:bold;margin-top:10px}</style></head><body><div class="box"><h1>LIFELINE INVESTMENTS</h1><h3>Create Account</h3><input id="name" placeholder="Full Name"><input id="phone" placeholder="Phone 07..."><input id="pass" type="password" placeholder="Password"><button onclick="reg()">Register Now</button><p id="msg"></p><br><h3>Login</h3><input id="lphone" placeholder="Phone"><input id="lpass" type="password" placeholder="Password"><button onclick="log()">Login</button><p id="msg2"></p></div><script>
async function reg(){
 ...
 if(r.ok) alert('SUCCESS: '+d.message);
}
async function log(){
 ...
 if(r.ok) alert('Welcome '+d.user.fullName);
}
</script></body></html>`);
});
...
app.post('/api/login', (req,res)=>{
  ...
  pool.query('SELECT * FROM users WHERE phone=?',[phone], async (err,results)=>{
    ...
    res.json({message:'Login success', user:{id:user.id, fullName:user.fullName, phone:user.phone}});
  });
});
