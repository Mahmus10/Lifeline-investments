...
app.get('/api/team/:id',async(req,res)=>{
 try{
  ...
  res.json({code:code, bonus:u[0].referralBonus||0, team:team, count:team.length});
...
app.get('/api/admin/deposits',async(req,res)=>{
 ...
 try{const[r]=await db.query("SELECT * FROM deposits WHERE status='pending' ORDER BY id DESC LIMIT 50");res.json(r);}catch(e){res.json([])}
});
...
app.get('/',(req,res)=>{res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:20px}input,button{width:100%;padding:14px;margin:8px 0;border-radius:10px;border:none}button{background:gold;font-weight:bold}</style></head><body><h2>Lifeline</h2><input id="n" placeholder="Name"><input id="p" placeholder="Phone"><input id="pw" type="password" placeholder="Password"><input id="rf" placeholder="Referral Code"><button onclick="reg()">Register</button><button onclick="log()" style="background:#333;color:#fff">Login</button><script>let code=new URLSearchParams(location.search).get("ref");if(code)rf.value=code;async function reg(){let r=await fetch("/api/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n.value,phone:p.value,password:pw.value,ref:rf.value})});let j=await r.json();if(j.id){localStorage.setItem("uid",j.id);location.href="/dashboard"}else alert(j.error)}async function log(){let r=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:p.value,password:pw.value})});let j=await r.json();if(j.id){localStorage.setItem("uid",j.id);location.href="/dashboard"}else alert("Wrong")}</script></body></html>');});
app.get('/dashboard',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;font-family:Arial;padding:15px}button{width:100%;padding:14px;margin:6px 0;border-radius:10px;border:none;font-weight:bold}.gold{background:gold}.dark{background:#222;color:#fff}.bal{background:#111;padding:15px;border-radius:15px;text-align:center}</style></head><body><div class="bal"><h1 id="b">0 UGX</h1><p id="ph"></p><p>Interest: <span id="int" style="color:#0f0">0</span></p><p id="code" style="color:gold;font-size:12px"></p></div><button class="gold" onclick="location.href='/deposit'">Deposit</button><button class="gold" onclick="location.href='/invest'">Invest</button><button class="dark" onclick="withdraw()">Withdraw Interest</button><button class="dark" onclick="location.href='/myinvest'">My Invest</button><button class="dark" onclick="location.href='/referral'" style="background:gold;color:#000">My Team / Referral 10%</button><button class="dark" onclick="localStorage.clear();location.href='/'">Logout</button><script>let uid=localStorage.getItem('uid');if(!uid)location.href='/';async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();b.textContent=(u.balance||0)+' UGX';ph.textContent=u.phone;int.textContent=(u.totalInterest||0)+' UGX';code.textContent='Ref: '+(u.myReferralCode||'')+' 10% Bonus';}async function withdraw(){let a=prompt('Amount min 5000');if(!a)return;let r=await fetch('/api/withdraw',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:uid,amount:parseInt(a)})});let j=await r.json();alert(j.error||'Sent');}load()</script></body></html>`);});
app.get('/deposit',(req,res)=>{res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;padding:20px;font-family:Arial}input,button{width:100%;padding:14px;margin:8px 0;border-radius:10px;border:none}button{background:gold;font-weight:bold}</style></head><body><a href="/dashboard" style="color:gold">Back</a><h2>Deposit Min 2000</h2><input id="am" placeholder="Amount min 2000"><input id="air" placeholder="Airtel No"><input type="file" id="file" accept="image/*"><img id="prev" style="display:none;width:100%"><button onclick="dep()">Submit</button><script>let uid=localStorage.getItem("uid");let b64="";file.addEventListener("change",e=>{let r=new FileReader();r.onload=()=>{b64=r.result;prev.src=b64;prev.style.display="block"};r.readAsDataURL(e.target.files[0])});async function dep(){let r=await fetch("/api/deposit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:uid,amount:am.value,airtelNo:air.value,screenshot:b64})});let j=await r.json();if(j.ok){alert("Sent!");location.href="/dashboard"}else alert(j.error)}</script></body></html>');});
app.get('/invest',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;padding:15px;font-family:Arial}.club{background:#111;padding:15px;border-radius:12px;margin:10px 0}button{width:100%;padding:12px;border-radius:8px;border:none;background:gold;font-weight:bold}input{width:100%;padding:12px;border-radius:8px;border:none}a{color:gold}</style></head><body><a href="/dashboard">Back</a><h2>Invest - Min 2000</h2><p>Bal: <span id="bal">0</span></p><div class="club"><h3>Arsenal 10%/10d</h3><input id="a-arsenal" placeholder="Min 2000"><button onclick="inv('arsenal')">Invest</button></div><div class="club"><h3>Man Utd 10%/10d</h3><input id="a-manutd" placeholder="Min 2000"><button onclick="inv('manutd')">Invest</button></div><div class="club"><h3>Man City 10%/10d</h3><input id="a-mancity" placeholder="Min 2000"><button onclick="inv('mancity')">Invest</button></div><div class="club"><h3>Chelsea 8%/8d</h3><input id="a-chelsea" placeholder="Min 2000"><button onclick="inv('chelsea')">Invest</button></div><div class="club"><h3>Liverpool 8%/8d</h3><input id="a-liverpool" placeholder="Min 2000"><button onclick="inv('liverpool')">Invest</button></div><script>let uid=localStorage.getItem('uid');async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();bal.textContent=u.balance}async function inv(c){let v=document.getElementById('a-'+c).value;if(parseInt(v)<2000)return alert('Min 2000');let r=await fetch('/api/invest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:uid,club:c,amount:parseInt(v)})});let j=await r.json();if(j.ok){alert('Invested!');location.href='/myinvest'}else alert(j.error)}load()</script></body></html>`);});
app.get('/myinvest',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;padding:15px;font-family:Arial}.card{background:#111;padding:12px;margin:10px 0;border-radius:10px}a{color:gold}</style></head><body><a href="/dashboard">Back</a><h2>My Invest</h2><div id="l"></div><script>let uid=localStorage.getItem('uid');const rates={arsenal:10,manutd:10,mancity:10,chelsea:8,liverpool:8};async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();let html='';let now=new Date();for(let i of (u.investments||[])){let d=Math.floor((now-new Date(i.startDate))/(1000*60*60*24)); if(d<0)d=0; let rate=i.rate||rates[i.club]||10; let lock=i.lockDays||(rate>=10?10:8); let p=Math.floor((parseFloat(i.amount)||0)*rate/100*d); html+='<div class=card><b>'+i.club.toUpperCase()+'</b> '+i.amount+' UGX<br>Profit: '+p+' UGX<br>Days: '+d+'/'+lock+'<br>Rate: '+rate+'% daily</div>'}l.innerHTML=html||'None'}load()</script></body></html>`);});
app.get('/referral',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;padding:15px;font-family:Arial}a{color:gold}.card{background:#111;padding:15px;border-radius:12px;margin:10px 0;text-align:center}button{width:100%;padding:12px;border-radius:8px;border:none;background:gold;font-weight:bold;margin:5px 0}input{width:100%;padding:12px;border-radius:8px;border:none;background:#222;color:#fff;text-align:center}</style></head><body><a href="/dashboard">Back</a><h2>My Referral Team</h2><div class="card"><h3 style="color:gold">Your Code: <span id="code">---</span></h3><p>Bonus Earned: <span id="bonus" style="color:#0f0">0 UGX</span></p><p>Team Count: <span id="count">0</span> members</p><input id="link" readonly><button onclick="copy()">Copy Referral Link</button><button onclick="share()" style="background:#25D366">Share on WhatsApp</button><p style="font-size:12px;color:#aaa">You get 10% of every deposit your team makes!</p></div><div id="team"></div><script>let uid=localStorage.getItem('uid');async function load(){let r=await fetch('/api/team/'+uid);let j=await r.json();code.textContent=j.code;bonus.textContent=(j.bonus||0)+' UGX';count.textContent=j.count;let url=location.origin+'/?ref='+j.code;link.value=url;let html='';if(j.team.length==0)html='<p style=text-align:center;color:#666>No team yet</p>';else for(let t of j.team){html+='<div class=card style=text-align:left><b>'+(t.fullName||t.phone)+'</b><br>Phone: '+t.phone+'<br>Balance: '+(t.balance||0)+' UGX</div>'}team.innerHTML=html}function copy(){link.select();document.execCommand('copy');alert('Copied!')}function share(){let m='Join Lifeline Investments! Use my code '+code.textContent+' Link: '+link.value; window.open('https://wa.me/?text='+encodeURIComponent(m))}load()</script></body></html>`);});
