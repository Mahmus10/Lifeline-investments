...
function genMatchday(){
  ...
  for(let i=0;i<8;i++){
    ...
    fixtures.push({id:Date.now()+i*1000+Math.floor(Math.random()*9000), home, away, odds:{home:'2.00',draw:'2.00',away:'2.00'}, targetResult:null, result:null});
  }
  return fixtures;
}
...
async function decideAllResults(fixtures){
  try{
    ...
    let res=[];
    for(let f of fixtures){ let t=totals[f.id]||{home:0,draw:0,away:0}; if(Math.random()<0.75){ let min=Math.min(t.home||0,t.draw||0,t.away||0); let cands=Object.keys(t).filter(k=>t[k]===min); res.push(cands[0]||'draw'); } else { let max=Math.max(t.home,t.draw,t.away); res.push(Object.keys(t).find(k=>t[k]===max)||randomResult()); } }
    return res;
...
async function settleMatchday(fixtures){
  ...
  try{ if(db){ const [accas]=await db.query("SELECT * FROM virtual_accas WHERE season=? AND matchday=? AND status='pending'",[season, matchday]); for(let acca of accas){ let sels=JSON.parse(acca.selections); let win=true; for(let s of sels){ let found=fixtures.find(f=> f.id===s.matchId); if(!found || found.result!==s.prediction){ win=false; break; } } if(win){ let w=Math.floor(acca.amount*acca.odd); await db.query("UPDATE users SET gameBalance=gameBalance+? WHERE id=?",[w, acca.userId]); await db.query("UPDATE virtual_accas SET status='won', winAmount=? WHERE id=?",[w, acca.id]); } else await db.query("UPDATE virtual_accas SET status='lost', winAmount=0 WHERE id=?",[acca.id]); } } }catch(e){}
  matchday++; if(matchday>30){matchday=1; season++;} nextFixtures=genMatchday();
}
setInterval(async()=>{
  virtualTimeLeft--;
  ...
  } else if(virtualPhase==='live' && virtualTimeLeft<=0){ await settleMatchday(liveFixtures); virtualPhase='betting'; virtualTimeLeft=118; liveFixtures=null; }
},1000);
async function init(){
 try{ const u=process.env.DATABASE_URL||process.env.MYSQL_URL; if(!u) throw new Error("no db"); db=mysql.createPool(u+"?connectionLimit=10&keepAlive=true");
  ...
  console.log("READY MD0 FIELD BG + FIXED TIMER");
 }catch(e){ console.log("memory"); }
}
init();
...
const pages = {
home: `<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="manifest" href="/manifest.json"><style>${FIELD_BG_CSS}input,button{width:100%;padding:14px;margin:8px 0;border-radius:12px;border:none}button{background:gold;font-weight:bold}.card{padding:20px;border-radius:20px}</style></head><body><div class="field-logo-top">MD 0 FRESH • x2.00 • STADIUM</div><div class="card glass" style="margin-top:60px"><h2>Lifeline</h2><input id="n" placeholder="Full Name"><input id="p" placeholder="Phone"><input id="pw" type="password" placeholder="Password"><button onclick="reg()">Register</button><button onclick="log()" style="background:#222;color:#fff">Login</button></div><script>async function reg(){let r=await fetch("/api/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n.value,phone:p.value,password:pw.value})});let j=await r.json();if(j.id){localStorage.setItem("uid",j.id);location.href="/dashboard"}else alert(j.error)}async function log(){let r=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:p.value,password:pw.value})});let j=await r.json();if(j.id){localStorage.setItem("uid",j.id);location.href="/dashboard"}else alert("Wrong")}</script></body></html>`,
dash: `<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="manifest" href="/manifest.json"><style>${FIELD_BG_CSS}.bal{padding:22px;border-radius:24px;text-align:center}.walletGrid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0}.walletCard{padding:14px;border-radius:16px;text-align:left}button{width:100%;padding:14px;margin:7px 0;border-radius:12px;border:none;font-weight:bold}.gold{background:gold;color:#000}.dark{background:rgba(20,25,35,0.9);color:#fff;border:1px solid rgba(255,255,255,0.1)}#wheelModal{position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:300;display:none;align-items:center;justify-content:center;padding:20px;flex-direction:column}#wheelCanvas{border-radius:50%;box-shadow:0 0 40px gold;border:6px solid gold;max-width:90vw}.virtCard{background:linear-gradient(135deg,#1a0033,#000033);border:2px solid #ff00cc;padding:16px;border-radius:18px;margin:12px 0}</style></head><body><div class="field-logo-top">MD 0 FRESH • x2.00 • STADIUM</div><div id="wheelModal"><canvas id="wheelCanvas" width="340" height="340"></canvas><button onclick="doSpin()" class="gold" style="max-width:340px;margin-top:15px">SPIN NOW</button><button onclick="closeWheel()" style="background:#333;color:#fff;max-width:340px">Close</button></div><div class="bal glass" style="margin-top:60px"><div class="walletGrid"><div class="walletCard glass"><h4>GAME</h4><h2 id="gb">0</h2></div><div class="walletCard glass"><h4>INVEST</h4><h2 id="b">0</h2></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div class="glass" style="padding:10px;border-radius:12px;text-align:center"><small>INTEREST</small><br><span id="int" style="color:#00ff88;font-weight:bold">0</span></div><div class="glass" style="padding:10px;border-radius:12px;text-align:center"><small>MINING</small><br><span id="mineBal" style="color:#ff00cc;font-weight:bold">0</span></div></div><p id="ph" style="color:gold"></p></div><div class="virtCard" onclick="location.href='/virtual'"><div style="display:flex;justify-content:space-between"><b style="color:#ff00cc">🔥 VIRTUAL x2.00 - MD 0</b><span id="virtPhase" style="background:#00ff88;color:#000;padding:4px 10px;border-radius:20px;font-size:10px">BETTING</span></div><div style="text-align:center;margin-top:8px">⏱️ <span id="virtTimer">--:--</span> • Tap →</div></div><button class="gold" onclick="openWheel()">🎡 SPIN WHEEL</button><button class="dark" onclick="startMining()">⛏️ Start Mining</button><button class="gold" onclick="location.href='/virtual'">⚽ PLAY VIRTUAL</button><button class="dark" onclick="location.href='/deposit'">💰 Deposit</button><button class="dark" onclick="location.href='/invest'">📈 Invest</button><button class="dark" onclick="location.href='/history'">📜 History</button><button class="dark" onclick="location.href='/referral'">👥 My Team</button><script>let uid=localStorage.getItem("uid");if(!uid)location.href="/";function openWheel(){ document.getElementById('wheelModal').style.display='flex'; let c=document.getElementById('wheelCanvas'); let ctx=c.getContext('2d'); let prizes=[0,100,200,300,500,1000,2000,100]; let angle=0; for(let i=0;i<prizes.length;i++){ ctx.beginPath(); ctx.moveTo(170,170); ctx.fillStyle=['#222','#ffcc00','#00ff88','#ff00cc','#00ccff','#ff6600','#ffd700','#444'][i]; ctx.arc(170,170,160, angle, angle+Math.PI*2/8); ctx.lineTo(170,170); ctx.fill(); ctx.save(); ctx.translate(170,170); ctx.rotate(angle+Math.PI/8); ctx.fillStyle='#fff'; ctx.font='bold 14px Arial'; ctx.fillText(prizes[i]+' UGX', 60, 5); ctx.restore(); angle+=Math.PI*2/8; } }function closeWheel(){document.getElementById('wheelModal').style.display='none';}async function doSpin(){let r=await fetch("/api/spin",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:uid})});let j=await r.json();alert(j.win!==undefined?"Won "+j.win:j.error);closeWheel();load();}async function startMining(){await fetch("/api/mining/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:uid})});alert("Mining!");load();}async function load(){try{let r=await fetch("/api/user/"+uid);let u=await r.json();b.textContent=(u.balance||0).toLocaleString();gb.textContent=(u.gameBalance||0).toLocaleString();mineBal.textContent=(u.miningBalance||0).toLocaleString();ph.textContent=u.phone||"";int.textContent=(u.totalInterest||0).toLocaleString();}catch(e){}}async function loadVirtualPreview(){try{let r=await fetch("/api/virtual/status");let d=await r.json();virtPhase.textContent=d.phase.toUpperCase();virtTimer.textContent=Math.floor(d.timeLeft/60)+":"+String(d.timeLeft%60).padStart(2,'0');}catch(e){}}load();setInterval(loadVirtualPreview,1000);loadVirtualPreview();</script></body></html>`,

// VIRTUAL WITH FIELD BACKGROUND + REAL CLOCK TIMER COUNTING 1 BY 1
...
.topBar{background:rgba(30,30,30,0.9);backdrop-filter:blur(10px);padding:14px 16px;display:flex;justify-content:space-between;position:sticky;top:0;z-index:20;border-bottom:1px solid rgba(255,255,255,0.1)}
.countDown{color:#ff6b00;font-weight:900;font-size:18px;letter-spacing:1px}
.matchCard{background:rgba(30,30,30,0.85);backdrop-filter:blur(12px);margin:10px 12px;border-radius:12px;padding:14px;border:1px solid rgba(255,255,255,0.08);box-shadow:0 4px 20px rgba(0,0,0,0.5)}
.teams{font-weight:800;font-size:14px;margin-bottom:12px;color:#fff}
.oddsRow{display:flex;gap:8px}
.oddBox{flex:1;background:rgba(50,50,50,0.9);border-radius:8px;padding:14px 10px;display:flex;justify-content:space-between;align-items:center;cursor:pointer;border:2px solid transparent;font-weight:bold;transition:0.15s}
.oddBox:active{transform:scale(0.96)}
.oddBox.sel{background:#1a3d1a;border-color:#00c853;box-shadow:0 0 15px rgba(0,200,83,0.3)}
.betSlipBar{position:fixed;bottom:0;left:0;right:0;background:#b2ff00;color:#000;padding:16px 16px;display:flex;justify-content:space-between;z-index:30;font-weight:900;font-size:14px;cursor:pointer;box-shadow:0 -5px 20px rgba(0,0,0,0.5)}
...
<div class="field-logo-top">MD 0 FRESH • STADIUM FIELD • x2.00</div>
<div class="topBar" style="margin-top:50px"><a href="/dashboard" style="color:#fff;text-decoration:none;font-size:24px">‹</a><span id="mdText" style="font-weight:700">Matchday 0 • Starts in: <span id="timer" class="countDown">01:58</span> • All x2.00</span><span id="liveDot" style="width:10px;height:10px;background:#0f0;border-radius:50%;display:inline-block;animation:blink 1s infinite"></span></div>
<div id="list" style="margin-top:8px">Loading 8 matches on stadium...</div>
<div class="betSlipBar" onclick="openSlip()"><span>🎫 Add legs x2.00 - Win Bonus</span><span id="legsCount" style="background:#000;color:#b2ff00;padding:6px 14px;border-radius:20px;font-size:13px">0 legs</span></div>
...
let selections=[];
let curMD=0;
let endTime = Date.now() + 118*1000; // REAL CLOCK DEADLINE - FIXES FREEZE
let FALLBACK=[
...
// REAL 1-BY-1 SECOND COUNTDOWN - USES CLOCK NOT COUNTER
function startRealTimer(){
  setInterval(()=>{
    let remain = getRemaining();
    document.getElementById('timer').textContent=fmt(remain);
    document.getElementById('mdText').innerHTML='Matchday '+curMD+' • Starts in: <span class=countDown>'+fmt(remain)+'</span> • All x2.00';
  }, 250); // update 4x per sec for smooth 1-by-1
}
...
app.listen(process.env.PORT||3000,()=>console.log("FIXED REAL CLOCK TIMER 1-BY-1 + FIELD BG BEHIND BETS - MD0"));
