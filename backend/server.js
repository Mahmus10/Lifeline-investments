...
const pages={
home:`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>${BG}input,button{width:100%;padding:14px;margin:8px 0;border-radius:12px;border:none;box-sizing:border-box}button{background:gold;font-weight:bold}.card{padding:20px;border-radius:20px}.err{color:#ff4444;font-size:12px;display:none;margin:-4px 0 8px 4px}</style></head><body><div class="top">LIFELINE • x2.00</div><div class="card glass" style="margin-top:60px"><h2>Register - ${MERCHANT_NAME}</h2><input id="n" placeholder="Full Name"><input id="p" placeholder="Phone"><input id="pw" type="password" placeholder="Password"><input id="cpw" type="password" placeholder="Confirm Password"><div id="err" class="err">❌ Passwords don't match!</div><button onclick="reg()">Register</button><button onclick="log()" style="background:#222;color:#fff">Login</button></div><script>
function checkMatch(){let a=pw.value, b=cpw.value; if(b.length>0 && a!==b){err.style.display='block'; return false;} else {err.style.display='none'; return true;}}
cpw.addEventListener('input', checkMatch); pw.addEventListener('input', checkMatch);
async function reg(){
 if(!n.value.trim()||!p.value.trim()||!pw.value.trim()){return alert("Fill all fields");}
 if(pw.value!==cpw.value){err.style.display='block'; return alert("Passwords don't match!");}
 if(pw.value.length<4){return alert("Password too short - min 4 chars");}
 let r=await fetch("/api/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n.value,phone:p.value,password:pw.value})});
 let j=await r.json(); if(j.id){localStorage.setItem("uid",j.id);location.href="/dashboard"} else alert(j.error)
}
async function log(){let r=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:p.value,password:pw.value})});let j=await r.json();if(j.id){localStorage.setItem("uid",j.id);location.href="/dashboard"} else alert("Wrong password or phone")}
<\/script></body></html>`,
...
1. Dial <b style="color:#fff">*185*9#</b><br>
2. Merchant Code: <b style="color:gold">${MERCHANT_CODE}</b><br>
3. Name: <b style="color:#0f0">${MERCHANT_NAME}</b><br>
4. Amount (min 1000)<br>
5. PIN to confirm<br>
6. Screenshot SMS & upload below
</div>
</div>
...
<label style="font-size:12px;color:#aaa">Your Airtel Number</label>
<input id="air" placeholder="e.g. 0750XXXXXX">
<label style="font-size:12px;color:#aaa">Payment Screenshot (No reuse!)</label>
<input type="file" id="file" accept="image/*">
<div id="fileName" style="font-size:11px;color:#0f0;margin:4px 0"></div>
<div class="warn">⚠️ Same screenshot cannot be used twice! System blocks duplicate screenshots.</div>
...
file.addEventListener("change", e=>{
 ...
 let r=new FileReader(); r.onload=()=>{ b64=r.result; let str=b64.substring(0,200); let hash=0;for(let i=0;i<str.length;i++){hash=((hash<<5)-hash)+str.charCodeAt(i);hash=hash&hash;} fileHash=Math.abs(hash).toString()+b64.length; }; r.readAsDataURL(f);
});
function copyCode(){navigator.clipboard.writeText("${MERCHANT_CODE}");alert("Merchant ${MERCHANT_CODE} - ${MERCHANT_NAME} copied!");}
async function loadDeps(){ try{ let r=await fetch("/api/deposits/"+uid); let d=await r.json(); if(!d.length){depList.innerHTML='<div style="color:#666">No deposits yet</div>';return;} let h="";d.slice(0,5).forEach(x=>{ let col=x.status==='pending'?'#ff0':x.status==='approved'?'#0f0':'#f44'; h+='<div style="background:#1a1a1a;padding:10px;margin:6px 0;border-radius:8px;display:flex;justify-content:space-between"><span>'+x.amount+' UGX - '+x.airtelNo+'</span><span style="color:'+col+'">'+x.status.toUpperCase()+'</span></div>'; }); depList.innerHTML=h; }catch(e){}}
async function dep(){
 let amt=am.value.trim(); let airtel=air.value.trim();
 ...
 try{
  ...
  let j=await r.json();
  if(j.ok){ msg.textContent="✅ Submitted to ${MERCHANT_CODE}! Admin will approve shortly."; msg.style.color="#0f0"; setTimeout(()=>{location.href="/dashboard";},1500); } else { msg.textContent="❌ "+(j.error||"Failed"); msg.style.color="#f44"; subBtn.textContent="✅ Submit Deposit";subBtn.disabled=false; }
 }catch(e){ msg.textContent="Network error: "+e.message;msg.style.color="#f44"; subBtn.textContent="✅ Submit Deposit";subBtn.disabled=false; }
}
loadDeps();
</script></body></html>`,
...
app.listen(process.env.PORT||3000,()=>console.log("FINAL+CONFIRM: Merchant ${MERCHANT_CODE} ${MERCHANT_NAME} + confirm pass"));
