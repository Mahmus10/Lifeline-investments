...
// REDUCED 0-0: Higher goal chance + force goal at end
function maybeGoal(){
...
// At end of match, if still 0-0, 75% chance to force a goal to reduce 0-0 percentage
fixtures.forEach(f=>{
...
// Another 50% of remaining 0-0 become 1-1 to reduce 0-0 further
...
<div class="top">MD <span id="topMD">0</span> • S<span id="topSeason">1</span> • FEW DRAWS</div>
...
<div id="nextSec" style="display:none"><div style="padding:12px;color:#fff;text-align:center;background:#1a2a1a;margin:0 12px;border-radius:10px;border:1px solid #b2ff00">🔥 Next MD <span id="nextMD">3</span> - BET NOW while Live is ON! Next in <span id="nextCountdown" style="color:#b2ff00">--:--</span><div style="font-size:11px;color:#aaa;margin-top:4px">Balance: Game + Invest usable</div></div><div id="nextList"></div></div>
<div id="resultsSec" style="display:none"><div style="padding:12px;color:#888;text-align:center">Results - Reduced 0-0 (now ~20% only)</div><div id="resList">No results</div></div>
...
if(a>userTotal)return alert("Not enough balance! Game + Invest = "+userTotal);
try{
...
let j=await r.json();if(j.ok){alert("Bet placed MD"+j.md+" x"+j.odd+" | Balance used: Game+Invest");sel=[];currentBetMD=0;rend();closeS();}else alert(j.error);
...
deposit:`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>${BG}input,button{width:100%;padding:14px;margin:8px 0;border-radius:10px;border:none}button{background:gold;font-weight:bold}</style></head><body><div class="top">DEPOSIT → VIRTUAL</div><a href="/dashboard" style="color:gold;margin-top:60px;display:inline-block">← Back</a><h2>Deposit - Usable for Virtual!</h2><div style="background:rgba(0,255,0,0.1);border:1px solid #0f0;padding:10px;border-radius:10px;font-size:12px;margin-bottom:10px">✅ Your deposit can now be used for Virtual betting directly!</div><input id="am" placeholder="Amount"><input id="air" placeholder="Airtel"><input type="file" id="file"><button onclick="dep()">Submit</button><script>let uid=localStorage.getItem("uid");let b64="";file.addEventListener("change",e=>{let r=new FileReader();r.onload=()=>{b64=r.result};r.readAsDataURL(e.target.files[0])});async function dep(){let r=await fetch("/api/deposit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:uid,amount:am.value,airtelNo:air.value,screenshot:b64})});let j=await r.json();if(j.ok)location.href="/dashboard"}</script></body></html>`,
...
if(totalBal<amount)return res.status(400).json({error:"Not enough! Total (Game+Invest)="+totalBal});
...
res.json({ok:1,odd,md:targetMD,usedGame:useGame,usedMain:useMain});
...
app.listen(process.env.PORT||3000,()=>console.log("FIXED: 75% less 0-0, deposit usable for virtual"));
