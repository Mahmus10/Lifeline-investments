...
function maybeGoal(){if(!liveFixtures)return;if(Math.random()<0.07){let cands=liveFixtures.filter(f=>{let [h,a]=f.score.split('-').map(Number);return (h+a)<5;});if(!cands.length)return;let f=cands[Math.floor(Math.random()*cands.length)];let [h,a]=f.score.split('-').map(Number);if(Math.random()<0.65){if(Math.random()<0.5)h++;else a++;f.score=h+'-'+a;f.result=scoreToResult(f.score);}}}
setInterval(async()=>{
virtualTimeLeft--;
...
<div id="nextSec" style="display:none"><div style="padding:12px;color:#fff;text-align:center;background:#1a2a1a;margin:0 12px;border-radius:10px;border:1px solid #b2ff00">🔥 Next MD <span id="nextMD">3</span> - BET NOW while Live is ON! Next in <span id="nextCountdown" style="color:#b2ff00;font-weight:900">--:--</span></div><div id="nextList"></div><div style="padding:10px 14px;color:#aaa;font-size:12px;text-align:center">You can bet on Next MD even when current MD is LIVE</div></div>
<div id="resultsSec" style="display:none"><div style="padding:12px;color:#888;text-align:center">Previous Results - Max 5 goals</div><div id="resList">No results</div></div>
...
<script>
let sel=[];let curPhase='betting';let curTab='live';let lastScores={};let currentBetMD=0;
...
curTab=t;
document.querySelectorAll('.tab').forEach(x=>{x.classList.remove('active');x.classList.remove('liveActive');});
let el=document.getElementById('tab'+t.charAt(0).toUpperCase()+t.slice(1));
el.classList.add('active');if(t==='live')el.classList.add('liveActive');
document.getElementById('liveSec').style.display=t==='live'?'block':'none';
document.getElementById('nextSec').style.display=t==='next'?'block':'none';
document.getElementById('resultsSec').style.display=t==='results'?'block':'none';
if(t==='results')loadResults();
if(t==='next'){sel=[];rend();currentBetMD=parseInt(topMD.textContent)+1;}
else if(t==='live'){sel=[];rend();}
}
function tog(id,hn,an,hs,as,p,md){
let ex=sel.findIndex(s=>s.matchId===id);
if(ex!==-1){if(sel[ex].prediction===p)sel.splice(ex,1);else sel[ex].prediction=p;}
else{
if(sel.length>=8){alert("Max 8");return;}
if(sel.length>0 && sel[0].md!==md){alert("Bet only one MD at a time! Clear betslip first");return;}
sel.push({matchId:id,homeTeam:hn,awayTeam:an,homeShort:hs,awayShort:as,prediction:p,md:md});
}
currentBetMD=md;
rend();
}
...
sc.textContent=sel.length;lc.textContent=sel.length+" legs";
let o=sel.length?Math.pow(2,sel.length):0;if(sel.length===8)o=256;odd.textContent='x'+o;ob.textContent=o;
let a=parseInt(amt.value)||0;pot.textContent=Math.floor(a*o).toLocaleString()+" UGX";
betMD.textContent=currentBetMD||topMD.textContent;slMD.textContent=currentBetMD||topMD.textContent;
sl.innerHTML=sel.map(s=>'<div style="background:#2a2a2a;padding:10px;margin:5px 0;border-radius:8px;display:flex;justify-content:space-between"><span>MD'+s.md+' '+s.homeShort+'-'+s.awayShort+' → '+s.prediction.toUpperCase()+' @2.00</span><span style="color:red;cursor:pointer" onclick="remSel('+s.matchId+')">✕</span></div>').join('')||'<small style=color:#888>Tap 1 X 2 on any match - Live or Next</small>';
}
...
if(sel.length<1)return alert("Add at least 1 leg!");
...
let j=await r.json();if(j.ok){alert("Bet placed for MD"+j.md+" x"+j.odd);sel=[];currentBetMD=0;rend();closeS();}else alert(j.error);
...
document.getElementById(tbodyId).innerHTML=html||'<tr><td colspan="8" style="color:#666">No data</td></tr>';
}
...
else{tag='<span style="color:#555;font-size:11px">MD'+md+' BETTING x2.00</span>';}
...
function renderNext(fix,md){
let h='';fix.forEach(m=>{
h+='<div class=match><div style="display:flex;justify-content:space-between"><span style="font-weight:800">'+m.home.short+' - '+m.away.short+'</span><span style="color:#b2ff00;font-size:11px;font-weight:900">MD '+md+' • BET OPEN • x2.00</span></div><div class=row style="margin-top:10px"><div class="odd" data-m="'+m.id+'" data-p="home" onclick="tog('+m.id+",'"+m.home.name+"','"+m.away.name+"','"+m.home.short+"','"+m.away.short+"','home',"+md+')"><span>1</span><span>2.00</span></div><div class="odd" data-m="'+m.id+'" data-p="draw" onclick="tog('+m.id+",'"+m.home.name+"','"+m.away.name+"','"+m.home.short+"','"+m.away.short+"','draw',"+md+')"><span>X</span><span>2.00</span></div><div class="odd" data-m="'+m.id+'" data-p="away" onclick="tog('+m.id+",'"+m.home.name+"','"+m.away.name+"','"+m.home.short+"','"+m.away.short+"','away',"+md+')"><span>2</span><span>2.00</span></div></div></div>';
});
document.getElementById('nextList').innerHTML=h;
}
...
else if(d.phase==='first_half'){disp=fmtUp(45-tl);label='🔴 LIVE 1ST HALF '+disp+' ↑ 0→45';liveCount.style.display='inline';}
else if(d.phase==='halftime'){disp="00:0"+tl;label='⏸️ HALFTIME '+disp+' ↓';liveCount.style.display='inline';}
else{let total=45+(45-tl);disp=fmtUp(total);label='🔴 LIVE 2ND HALF '+disp+' ↑ 45→90';liveCount.style.display='inline';}
...
if(curTab==='live'){
let fix=d.phase==='betting'?d.currentFixtures:d.allLive;
renderLive(fix,d.phase,tl,d.matchday);
}
...
season,
matchday:matchday+1,
nextMD: (matchday+1)%15+1,
phase:virtualPhase,
timeLeft:virtualTimeLeft,
currentFixtures: virtualPhase==='betting'?currentFixtures:[],
allLive:liveFixtures,
...
app.listen(process.env.PORT||3000,()=>console.log("LIVE + NEXT BETTING: Bet on Next MD while Live is ON + table + invest 10% 8%"));
