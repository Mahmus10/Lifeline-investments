...
pool.query(`CREATE TABLE IF NOT EXISTS deposits (
  id INT AUTO_INCREMENT PRIMARY KEY,
  userId INT, amount DECIMAL(10,2), status VARCHAR(20) DEFAULT 'pending',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)`);
pool.query(`CREATE TABLE IF NOT EXISTS investments (
  id INT AUTO_INCREMENT PRIMARY KEY,
  userId INT, plan VARCHAR(50), amount DECIMAL(10,2), profit DECIMAL(10,2),
...
async function reg(){const fullName=name.value,phone=document.getElementById('phone').value,password=pass.value;const r=await fetch('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fullName,phone,password})});const d=await r.json();msg.innerText=d.message;if(r.ok)alert(d.message)}
async function log(){const phone=lphone.value,password=lpass.value;const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,password})});const d=await r.json();msg2.innerText=d.message;if(r.ok){localStorage.setItem('user',JSON.stringify(d.user));location.href='/dashboard'}}
</script></body></html>`);
});

app.get('/dashboard', (req,res)=>{
  res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#0a0a0a;color:white;font-family:sans-serif;padding:20px;margin:0}.card{background:#1a1a1a;padding:20px;border-radius:15px;max-width:420px;margin:auto}h1{color:#00ff88}button{padding:12px 18px;margin:5px;border:none;border-radius:8px;font-weight:bold;cursor:pointer}.green{background:#00ff88}.red{background:#ff4444;color:white}.plan{background:#222;padding:12px;border-radius:10px;margin:10px 0;border:1px solid #00ff88}input{width:100%;padding:10px;margin:8px 0;border-radius:8px;border:none;box-sizing:border-box}#depositBox,#investBox{display:none;background:#222;padding:15px;border-radius:10px;margin-top:15px}</style></head><body><div class="card"><h1 id="welcome">Welcome</h1><p>Balance: <b>UGX <span id="bal">0</span></b></p><button class="green" onclick="showDeposit()">Deposit</button><button class="green" onclick="showInvest()">Invest</button><button class="red" onclick="logout()">Logout</button><div id="depositBox"><h3>Deposit via MTN/Airtel</h3><p>Send to: <b>0762123456 - Lifeline</b></p><input id="depAmount" type="number" placeholder="Amount UGX"><button class="green" onclick="doDeposit()">I have Paid</button><p id="depMsg"></p></div><div id="investBox"><h3>Investment Plans</h3><div class="plan"><b>Starter - 10% daily for 10 days</b><br>Min: 20,000 UGX<br><button class="green" onclick="doInvest('Starter',20000)">Invest 20k</button></div><div class="plan"><b>Pro - 15% daily for 15 days</b><br>Min: 50,000 UGX<br><button class="green" onclick="doInvest('Pro',50000)">Invest 50k</button></div><div class="plan"><b>VIP - 20% daily for 20 days</b><br>Min: 100,000 UGX<br><button class="green" onclick="doInvest('VIP',100000)">Invest 100k</button></div><p id="invMsg"></p></div><div id="myInv"></div></div><script>
...
async function doDeposit(){const amount=depAmount.value;if(!amount)return alert('Enter amount');const r=await fetch('/api/deposit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:u.id,amount})});const d=await r.json();depMsg.innerText=d.message;alert(d.message);}
...
app.post('/api/register', async (req,res)=>{
  ...
  pool.query('INSERT INTO users (fullName, phone, password) VALUES (?,?,?)',[fullName,phone,hashed],(err)=>{
    if(err){if(err.code==='ER_DUP_ENTRY') return res.status(400).json({message:'Phone already registered'}); return res.status(500).json({message:err.message});}
...
app.post('/api/deposit', (req,res)=>{
  ...
  pool.query('INSERT INTO deposits (userId, amount) VALUES (?,?)',[userId, amount], (err)=>{
    if(err) return res.status(500).json({message:'Deposit failed'});
    res.json({message:'Deposit request sent! Admin will approve in 10 mins. Amount: '+amount+' UGX'});
  });
});
app.post('/api/invest', (req,res)=>{
  const {userId, plan, amount}=req.body;
  pool.query('INSERT INTO investments (userId, plan, amount, profit) VALUES (?,?,?,?)',[userId, plan, amount, amount*0.1], (err)=>{
    if(err) return res.status(500).json({message:'Investment failed: '+err.message});
    res.json({message:'Invested in '+plan+' plan! You will earn daily. Good luck!'});
  });
});
