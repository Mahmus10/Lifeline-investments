...
async function init(){
 try{
  ...
  try{await db.query("ALTER TABLE investments ADD COLUMN amount INT")}catch(e){}
  try{await db.query("ALTER TABLE investments ADD COLUMN userId INT")}catch(e){}
  try{await db.query("ALTER TABLE investments ADD COLUMN startDate DATETIME DEFAULT CURRENT_TIMESTAMP")}catch(e){}
  try{await db.query("ALTER TABLE investments ADD COLUMN status VARCHAR(20) DEFAULT 'active'")}catch(e){}
  ...
  try{await db.query("UPDATE investments SET startDate=NOW() WHERE startDate IS NULL")}catch(e){}
...
app.get('/api/user/:id',async(req,res)=>{
 ...
 const rateMap={arsenal:10,manutd:10,mancity:10,chelsea:8,liverpool:8};
 for(let i of inv){
  let sd=new Date(i.startDate); let days=isNaN(sd)?0:Math.floor((now - sd)/(1000*60*60*24)); if(days<0)days=0;
  let r=i.rate||rateMap[i.club]||10; totalInterest+= Math.floor((i.amount||0) * r/100 * days);
 }
...
app.post('/api/invest',async(req,res)=>{
 try{
  ...
  await db.query("INSERT INTO investments (userId,club,amount,rate,lockDays,startDate,status) VALUES (?,?,?,?,?,NOW(),'active')",[userId,club,amount,clubs[club],locks[club]]);
  res.json({ok:1});
 }catch(e){res.status(400).json({error:e.message})}
});
app.post('/api/withdraw',async(req,res)=>{
 try{
  ...
  let totalInterest=0; let now=new Date();
  const rateMap={arsenal:10,manutd:10,mancity:10,chelsea:8,liverpool:8};
  for(let i of inv){ let sd=new Date(i.startDate); let days=isNaN(sd)?0:Math.floor((now - sd)/(1000*60*60*24)); let r=i.rate||rateMap[i.club]||10; totalInterest+= Math.floor((i.amount||0)*r/100*days); }
...
app.get('/myinvest',(req,res)=>{res.send(`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#000;color:#fff;padding:15px;font-family:Arial}.card{background:#111;padding:12px;margin:10px 0;border-radius:10px}a{color:gold}</style></head><body><a href="/dashboard">Back</a><h2>My Invest</h2><div id="l"></div><script>let uid=localStorage.getItem('uid');const rates={arsenal:10,manutd:10,mancity:10,chelsea:8,liverpool:8};async function load(){let r=await fetch('/api/user/'+uid);let u=await r.json();let html='';let now=new Date();for(let i of (u.investments||[])){let sd=new Date(i.startDate); let days=isNaN(sd.getTime())?0:Math.floor((now-sd)/(1000*60*60*24)); if(days<0||isNaN(days))days=0; let rate=i.rate||rates[i.club]||10; let lock=i.lockDays||(rate>=10?10:8); let amt=parseFloat(i.amount)||0; let p=Math.floor(amt*rate/100*days); html+='<div class=card><b>'+i.club.toUpperCase()+'</b> '+amt+' UGX<br>Profit: '+p+' UGX<br>Days: '+days+'/'+lock+'<br>Rate: '+rate+'% daily</div>'}l.innerHTML=html||'None'}load()</script></body></html>`);});
